...
/**
 * The ChatApplication class serves as the Model (in the sense of the common
...
public class ChatApplication extends Application implements Observable {
  ...
  /**
   * The object we use in notifications to indicate that an AllJoyn application has
   * ended.
   */
   ...
   */
  public static final String HOST_NICK_NAME_CHANGED_EVENT = "HOST_NICK_NAME_CHANGED_EVENT";
  ...
  /**
   * The object we use in notifications to indicate that the the user has
   * send a file and it is queued to be sent to the outside world.
   */
  public static final String SEND_FILES_CHANGED_EVENT = "SEND_FILES_CHANGED_EVENT";
  ...
  /**
   * The object we use in notifications to indicate that the file already sent
   * observers need to synchronize with it.
   */
  public static final String TOAST_SEND_FILES = "TOAST_SEND_FILES";

  /**
   * The object we use in notifications to indicate that the file already received
   * observers need to synchronize with it.
   */
  public static final String TOAST_RECEIVE_FILES = "TOAST_RECEIVE_FILES";

  private static final String TAG = "app.ChatApplication";
  public static String PACKAGE_NAME;
  public static String[] fileName, savedFileName;
  ...
  /**
   * Set the user nickname for chat.
   */
  public synchronized void setNickName(String name) {
    nickName = name;
    notifyObservers(HOST_NICK_NAME_CHANGED_EVENT);
  }
   ...
   * remote bus attachment, we need to tell the AllJoyn Service to leave
  ...
  public synchronized void newLocalUserMessage(String message) {
    addInboundItem("Me", "(" + nickName + ") " + message);
    if (useGetChannelState() == AllJoynService.UseChannelState.JOINED) {
      addOutboundItem("(" + nickName + ") " + message);
    }
  }

  /**
   * Whenever our local user send a file, we need to send it out on the
   * channel, which we do by calling addOutbondSendFiles. This will eventually
   * result in an AllJoyn Bus Signal being sent to the other participants on
   * the channel.
   */
  public synchronized void newLocalUserFiles(String[] filename, String[] savedFilename) {
    addInboundSendFiles(filename, savedFilename);
    if (useGetChannelState() == AllJoynService.UseChannelState.JOINED) {
      addOutbondSendFiles(filename, savedFilename);
    }
  }
  ...
  /**
   * Whenever a user send a file into the channel, we expect the AllJoyn
   * Service local to that user to send the file to everyone participating
   * on the channel. At each participant, the files arrive in the AllJoyn
   * Service as a Bus Signal. The Service handles the signals and passes the
   * associated files on to us here. We expect the nickname to be the
   * unique ID of the sending bus attachment. This is not very user friendly,
   * but is convenient and guaranteed to be unique.
   */
  public synchronized void newRemoteUserFiles(String[] filename, String[] savedFilename) {
    addInboundSendFiles(filename, savedFilename);
  }

  /**
   * Whenever the local user types a message for distribution to the channel
   * it calls newLocalUserMessage. We are called to queue up the message and
  ...
  /**
   * Whenever the local user send a file for distribution to the channel
   * it calls newLocalUserFiles. We are called to queue up the message and
   * send a notification to all of our observers indicating that the we have
   * something ready to go out. We expect that the AllJoyn Service will
   * eventually respond by calling back in here to get items off of the queue
   * and send them down the session corresponding to the channel.
   */
  private void addOutbondSendFiles(String[] filename, String[] savedFilename) {
    fileName = filename;
    savedFileName = savedFilename;
    notifyObservers(SEND_FILES_CHANGED_EVENT);
  }
  ...
  /**
   * Whenever a file comes in from the AllJoyn Service over its channel
   * session, it calls in here. We just record the filename provided by Service.
   * This is currently expected to be the unique name of the bus attachment
   * originating the filename. Once the filename is saved, a change
   * notification will be sent to all observers indicating that the filename has
   * changed. The user interface part of the application is then expected to
   * wake up and synchronize itself to the new filename.
   */
  private void addInboundSendFiles(String[] filename, String[] savedFilename) {
    fileName = filename;
    savedFileName = savedFilename;
  ...
  private void addHistoryItem(String nickname, String message) {
    ...
    mHistory.add("[" + dateFormat.format(date) + "] " + message);
    notifyObservers(HISTORY_CHANGED_EVENT);
  }
   ...
   * interesting event has happened, it called here and provides an object
  ...
  /**
   * Enumeration of the high-level modules in the system. There is one
   * value per module.
   */
